...
const Page = () => {
  ...
  useEffect(() => {
    async function getTask() {
      try {
        const { data } = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_BASEURL}/getTask/${id}`);
...
const {data} = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_BASEURL}/updateTask/${id}`,{
